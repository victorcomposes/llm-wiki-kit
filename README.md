...
| `council-gate` | PreToolUse on Edit/Write, user `settings.json` | Denies an edit to a migration or contract path unless a ruled `council.md` entry in an active ticket carries a `Clears:` glob matching the file; `/council` writes the ledger (three fresh-context seats judge the fork, decision recorded with dissent). Agents in `templates/agents/`. |
