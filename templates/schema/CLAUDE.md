...
- **`/grilling`** — stress-test a ticket's `plan.md` (or any design) before implementation; fold the surviving decisions back into `plan.md`.
...
- **`/diagnosing-bugs`** — disciplined debugging loop; its repros, root causes, and gotchas are exactly what the capture rule files into folder-notes, and its friction findings are prototype seeds.
...
- **Mark how you know it (standing rule).** Every substantive claim you file into the vault carries its provenance inline. Without one it is indistinguishable from verified fact the moment the session ends, and the next session - or the next agent - reads it as ground truth. Two forms only:
  - **Observed** - you ran it, read it, or reproduced it. Cite the proof: `<!-- observed YYYY-MM-DD: File.cs:41 -->`, the command, or a commit sha. "I checked" is not provenance.
  - **Inferred** - reasoned from a symptom, an error message, one grep hit, or a single source. It files as `> [!question] Unverified (YYYY-MM-DD): <claim>. Confirm by <the check that settles it>.` Never as an assertion.
  
  A verified inference is rewritten as observed, with its proof. An unverified one stays a question - that is the correct resting state, not a gap to tidy away.
- **Capture service knowledge as you go (standing rule).** Whenever you learn something substantive about a service while working — investigating a ticket, reading its code, debugging, or ingesting a source — file it into that service's folder-note (`wiki/services/<Name>/<Name>.md`) *before the session ends*, not only into the ticket. Ticket folders are work-scoped and ephemeral; the folder-note is the durable, discoverable home. Substantive means: how a subsystem works, a non-obvious code path, an architectural constraint, a relationship to another service, a gotcha or failure mode. Update the relevant section (`## Architecture`, `## Relationships`, `## Build & Test`, etc.), add a `> [!warning] Updated <date>` callout if it contradicts what's there, and link back to the ticket that surfaced it (`[[{{TICKET_PREFIX}}-NNNN]]`). Skip what the code already makes obvious or what's purely ticket-specific. File observations freely: if unsure whether an *observed* fact belongs on the folder-note, it probably does. Inferences are the opposite - they file only as `> [!question]` open questions, never as assertions, and not at all if you would not bother to verify them. Volume of observed facts is the goal; volume of unverified claims is the failure mode.
...
- Folder layout: `tickets/{{TICKET_PREFIX}}-NNNN/{state,context,plan,notes}.md`, plus a thin folder-note `{{TICKET_PREFIX}}-NNNN.md` (title + pointers to the work files, no status duplication) so `[[{{TICKET_PREFIX}}-NNNN]]` wikilinks resolve vault-wide — same pattern as service folder-notes. Completed tickets move to `tickets/_archive/{{TICKET_PREFIX}}-NNNN/` (see Completion below).
...
- Completion: set `status: done`, ensure `plan.md` reflects what shipped, append a `done` entry to `log.md`, then **`git mv tickets/{{TICKET_PREFIX}}-NNNN tickets/_archive/{{TICKET_PREFIX}}-NNNN`**.
- **Completed tickets live in `tickets/_archive/{{TICKET_PREFIX}}-NNNN/`.** A vault accumulates finished tickets fast, and every routine reader — session orientation, lint, any plan-gating hook — pays to walk them before filtering them out. Archiving is a path move, not a delete: `[[{{TICKET_PREFIX}}-NNNN]]` still resolves (Obsidian matches the path suffix, so `[[{{TICKET_PREFIX}}-NNNN/plan]]` works too), backlinks still work, and the files stay greppable when you deliberately go looking. Scope the routine readers to `tickets/*/` and let them skip the archive; keep an index at `tickets/_archive/_archive.md`. Use a leading underscore, **not** a dot — Obsidian ignores dot-folders entirely, which would break every wikilink into the archive. Reopening a ticket is `git mv` back plus `status: active`; never scaffold a fresh folder over an archived id, or the wikilink becomes ambiguous between two folder-notes.
...
- **The loop: Explore → Plan → Code → Commit (mandatory for any code change).** Every change to service code under `{{ROOT_DIR}}/<Service>/` runs these four phases in order, never skipping ahead:
  ...
  2. **Plan** — persist numbered steps to `tickets/{{TICKET_PREFIX}}-NNNN/plan.md` under `## Key Changes`, set the ticket's `state.md` to `status: active`, stress-test the draft (if `/grilling` is installed), and get the user's explicit approval. The plan is the success measure Code is checked against.
...
- **Assertions crossing an agent boundary carry their provenance.** A claim you send to another agent - cross-session message, subagent prompt, handoff doc - is one hop from being written into the vault by a reader who cannot see how you came by it. Mark it observed or inferred in the message itself, and for an inference name what you have not run. On the receiving side a bare assertion is **unverified by default**: verify it or refuse to file it, never launder it into your own output as fact.
...
- Never `git add -A` or `git add -A <dir>`. Stage the explicit paths the change touched. A blanket add has both swept in a stray file and committed a source file truncated to zero bytes.
- **In a worktree another agent session is also committing in, do not stage at all.** `git commit -m "{{TICKET_PREFIX}}-NNNN <summary>" -- <path> <path>` commits exactly the named paths from the working tree and ignores the shared index. Naming explicit paths to `git add` is *not* enough: two agent panes sharing one index will sweep each other's files into each other's commits, because `git status` is accurate when each reads it and stale by the time each commits. An untracked file has to be added first, so keep that one `git add` and the pathspec commit back to back, and verify with `git show --stat HEAD`. If a sweep is already committed and another session has committed on top, do not rewrite: report it and let the human decide, because the content is intact and only its attribution is wrong.
